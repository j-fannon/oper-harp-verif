...
sum_inverted_fss_ranks <- function(data_model, fs = "FSS"){
        tmp_rr <- data_model[[fs]]$ranking
...
main_ranking <- function(verif_data){
   ...
   for (model in names(verif_data)){
      ...
      verif_data[[model]][['fss_sum_rank']] <-
          sum_inverted_fss_ranks(verif_data[[model]],fs = "FSS")
      verif_data[[model]][['fssp_sum_rank']] <-
        sum_inverted_fss_ranks(verif_data[[model]],fs = "FSSp")
   }
   ...
   tmp <- as.numeric(lapply(verif_data, '[[', "fssp_sum_rank"))
   ranks_avg_fssp_rank <- rank_max(tmp) # with 1 being the best rank
   
   for (i in 1:length(verif_data)){
           verif_data[[i]][["rank_avg_rank"]] <- ranks_avg_rank[i]
           verif_data[[i]][["rank_avg_fss_rank"]] <- ranks_avg_fss_rank[i]
           verif_data[[i]][["rank_avg_fssp_rank"]] <- ranks_avg_fssp_rank[i]
   }
